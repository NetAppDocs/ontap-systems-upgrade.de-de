...
---
= Ein Upgrade-Verfahren für AFF, ASA oder FAS Controller-Hardware auswählen
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
Im Allgemeinen hängt die Aktualisierung der NetApp AFF-, ASA- oder FAS-Controller-Hardware von den Plattformmodellen der ursprünglichen Knoten ab. Die Aktualisierung erfolgt entweder durch das Verschieben von Aggregaten oder Volumes (beides unterbrechungsfreie Verfahren) oder durch das Verschieben des Speichers (ein unterbrechendes Verfahren). Wenn auf den ursprünglichen und neuen Knoten unterschiedliche ONTAP-Versionen ausgeführt werden, kann vor Beginn der Hardware-Aktualisierung ein Software-Upgrade erforderlich sein.
...
| Läuft auf dem ursprünglichen und dem neuen Knoten eine unterschiedliche Softwareversion? | Aktualisieren Sie die ONTAP-Version auf den ursprünglichen Knoten auf die höchste unterstützte Version. Wenn möglich, müssen die neuen Knoten die auf den ursprünglichen Knoten laufende ONTAP-Version unterstützen. Ist dies nicht möglich, link:https://docs.netapp.com/us-en/ontap/upgrade/prepare.html["Aktualisieren Sie die ONTAP-Version"^] auf den ursprünglichen Knoten auf die höchste unterstützte Version. Der Versionsunterschied zwischen den ursprünglichen und den neuen Knoten darf nicht größer als vier sein. Zum Beispiel werden ONTAP 9.8 und 9.12.1 unterstützt; jedoch werden ONTAP 9.8 und 9.13.1 nicht unterstützt. link:https://docs.netapp.com/us-en/ontap/upgrade/concept_mixed_version_requirements.html["Erfahren Sie mehr über ONTAP-Cluster verschiedener Versionen"^]. 


| Möchte ich ein disruptives Upgrade durchführen? | link:upgrade/upgrade-decide-to-use-this-guide.html["Upgrades durch Storage-Verschiebung"]. Dies erfordert ein neues System mit eigenem Speicher und eigenen Regalen zur Speicherung der Daten des ursprünglichen Systems. Für bestimmte Upgrades kann ein Controller-Regal umgerüstet werden. 
...
* Das ursprüngliche System in ein Laufwerksgehäuse umwandeln und den Speicher verschieben:
+
** link:upgrade/upgrade_aff_a250_to_aff_a400_ndu_upgrade_workflow.html["AFF A250 in ein Laufwerksgehäuse umwandeln und Speicher auf AFF A400 verschieben"]
** link:upgrade/convert-to-drive-shelf.html["Konvertieren Sie AFF A150, AFF A220 oder FAS2820 in ein Laufwerksgehäuse und verschieben Sie den Speicher auf ein unterstütztes Ersatzsystem"]
...
| Habe ich ein modulares System? *Hinweis*: Bei einem modularen System verfügt der Controller über keine internen Festplatten; der gesamte Speicher befindet sich in extern angeschlossenen Gehäusen. | link:upgrade-arl/index.html["Upgrade durch Aggregatverschiebung"]. 
...
| Habe ich ein integriertes System und möchte ich es aufrüsten, indem ich nur das Controller-Modul austausche und das vorhandene Gehäuse und die Festplatten beibehalte? | link:upgrade-arl-auto-in-chassis/index.html["Upgrade mittels ARL unter Beibehaltung des gleichen Gehäuses, der gleichen Festplatten und der auf den Festplatten gespeicherten Daten"]. 
...
Dieses Beispiel liefert mögliche Antworten für ein FAS9500-zu-FAS90-Controller-Upgrade.
