...
[role="lead"]
Node1 mit den Ersatzmodulen ist jetzt bereit zum Hochfahren. In diesem Abschnitt werden die Schritte beschrieben, die zum Starten von Knoten 1 mit den Ersatzmodulen für die folgenden Upgrade-Konfigurationen erforderlich sind:
...
|===
^1^ durch Austausch des Controller-Moduls werden alle Verbindungen vom alten zum Ersatz-Controller-Modul verschoben.
^2^ bei der Aktualisierung durch Austausch des Controllers und der NVRAM-Module werden nur die Konsole und Managementverbindungen verschoben.
...
.. Prüfen und setzen Sie die Storage Encryption Boot-Optionen zurück, die in erwähnt sind <<A900_boot_node1,Schritt 1>>.
