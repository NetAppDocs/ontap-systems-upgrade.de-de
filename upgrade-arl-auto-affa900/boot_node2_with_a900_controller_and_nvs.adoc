...
.Schritte
. [[A900_Boot_node2]] Wenn NetApp Storage Encryption (NSE) für diese Konfiguration verwendet wird, wird der aktiviert `setenv bootarg.storageencryption.support` Der Befehl muss auf festgelegt sein `true`, Und das `kmip.init.maxwait` Variable muss auf festgelegt sein `off` So vermeiden Sie eine Boot-Schleife nach dem Laden der node2-Konfiguration:
+
