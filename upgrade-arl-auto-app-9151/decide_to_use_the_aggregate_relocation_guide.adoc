...
[role="lead"]
Für die Aktualisierung der Controller-Hardware stehen verschiedene Methoden zur Aggregate-Umverlagerung (ARL) zur Verfügung. Dieses ARL-Verfahren beschreibt, wie ein HA-Controller-Paar in AFF und FAS Speichersystemen, die in ONTAP 9.15.1 oder später eingeführt wurden, mit neuen Controllern aktualisiert wird, wobei die vorhandenen Festplatten und gespeicherten Daten erhalten bleiben. Dies ist ein komplexes Verfahren, das nur von erfahrenen ONTAP Administratoren verwendet werden sollte.
