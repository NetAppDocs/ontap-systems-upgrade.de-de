...
[role="lead"]
Es gibt mehrere Methoden zur aggregierten Umverteilung (ARL) für die Aktualisierung der Controller-Hardware. Dieses ARL-Verfahren beschreibt, wie ein HA-Controller-Paar in AFF und FAS-Speichersystemen, die in ONTAP 9.15.1 oder höher eingeführt wurden, mit neuen Controllern aufgerüstet wird, während die vorhandenen Festplatten und die darauf gespeicherten Daten erhalten bleiben. Dies ist ein komplexes Verfahren, das nur von erfahrenen ONTAP Administratoren verwendet werden sollte.
