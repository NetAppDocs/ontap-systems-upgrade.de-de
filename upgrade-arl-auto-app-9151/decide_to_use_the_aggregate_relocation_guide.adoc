...
summary: 'Es ist zu entscheiden, ob ARL mithilfe von  `system controller replace` Befehlen verwendet werden soll, um auf Controller zu aktualisieren, die in ONTAP 9.15.1 oder später eingeführt wurden.' 
---
...
[role="lead"]
Für die Aktualisierung der Controller-Hardware stehen verschiedene Aggregate-Umverlagerungsmethoden (ARL) zur Verfügung. Dieses ARL-Verfahren beschreibt, wie ein HA-Paar von Controllern in AFF und FAS Storage-Systemen auf Controller aktualisiert wird, die in ONTAP 9.15.1 oder höher eingeführt wurden, wobei die vorhandenen Festplatten und die darauf gespeicherten Daten erhalten bleiben. Dies ist ein komplexes Verfahren, das nur von erfahrenen ONTAP Administratoren verwendet werden sollte.
...
|===
+
Um ein in der obigen Tabelle aufgeführtes Upgrade durchzuführen, siehe link:other-references.html["Quellen"] zum Link zu _Verwenden Sie die Befehle "system controller replace", um Controller-Modelle im selben Chassis zu aktualisieren_.

* Sie aktualisieren eine MetroCluster FC- oder IP-Konfiguration. Zum Upgrade einer MetroCluster Konfiguration siehe link:other-references.html["Quellen"] in der Dokumentation zu _MetroCluster Upgrade und Expansion_.
...
Nachdem wir uns für die Anwendung dieses ARL-Verfahrens entschieden haben, link:required_tools_and_documentation.html["Informieren Sie sich über die benötigten Werkzeuge und die Dokumentation"].
