...
summary: 'Aktivieren Sie das HA-Paar, überprüfen Sie den Storage-Zugriff von node 3 und node 4, und bestätigen Sie die Aggregateigentümerschaft, nachdem Sie die Controller mit ONTAP 9.15.1 oder höher mithilfe von Befehlen aktualisiert `system controller replace` haben.' 
---
...
+
Siehe link:other_references.html["Quellen"] zur _System Administration Reference_ für Informationen zu den SPs und zur _ONTAP 9 Command reference_ für detaillierte Informationen zum System `service-processor network modify`-Befehl.
