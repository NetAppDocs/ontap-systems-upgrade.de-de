...
summary: 'Das HA-Paar wird aktiviert, der Speicherzugriff von node3 und node4 überprüft und die Aggregate-Eigentümerschaft nach dem Upgrade auf Controller, die in ONTAP 9.15.1 oder höher eingeführt wurden, mithilfe von  `system controller replace` Befehlen bestätigt.' 
---
...
. [[unset_maxwait_System_commands]]Wenn in der Konfiguration selbstverschlüsselnde Laufwerke verwendet werden und Sie die Variable auf gesetzt haben `kmip.init.maxwait` `off` (z.B. in link:install_boot_node4.html#auto_install4_step24["installieren und booten sie node4, Schritt 24"]), müssen Sie die Einstellung der Variable aufheben:
+
...
Nachdem bestätigt wurde, dass die neuen Controller korrekt eingerichtet sind, link:set_up_storage_encryption_new_module.html["Speicherverschlüsselung auf dem neuen Controllermodul einrichten"].
