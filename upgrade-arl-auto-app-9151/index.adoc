...
[role="lead"]
Es gibt mehrere Methoden zur Aggregate-Umverlagerung (ARL) für die Aktualisierung der Controller-Hardware. Dieses ARL-Verfahren beschreibt, wie ein HA-Paar von Controllern in AFF- und FAS-Speichersystemen, die in ONTAP 9.15.1 oder später eingeführt wurden, mit neuen Controllern aufgerüstet wird, während die vorhandenen Festplatten und gespeicherten Daten erhalten bleiben.
