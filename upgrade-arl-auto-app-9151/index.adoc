...
[role="lead"]
Es gibt mehrere Methoden zur Aggregate-Umverlagerung (ARL) für die Aktualisierung der Controller-Hardware. Dieses ARL-Verfahren beschreibt, wie ein HA-Controller-Paar in AFF und FAS Storage-Systemen, die in ONTAP 9.15.1 oder später eingeführt wurden, mit neuen Controllern aktualisiert wird, während die vorhandenen Festplatten und die darauf gespeicherten Daten erhalten bleiben.


NOTE: Dieses Verfahren kann nicht zum Aktualisieren einer MetroCluster FC- oder IP-Konfiguration verwendet werden. Zum Aktualisieren einer MetroCluster Konfiguration siehe link:other-references.html["Quellen"] die Dokumentation zu _MetroCluster Upgrade und Expansion_.
...
Bei einem Controller-Upgrade wird der vorhandene Controller durch einen anderen Controller mit einer neueren oder leistungsstärkeren Plattform ersetzt. Dieser Artikel beschreibt die Schritte des teilautomatisierten Verfahrens, das automatische Netzwerkport-Erreichbarkeitsprüfungen nutzt, um das Controller-Upgrade weiter zu vereinfachen.

.Was kommt als Nächstes?
Nachdem Sie sich über dieses ARL-Upgrade-Verfahren informiert haben, link:decide_to_use_the_aggregate_relocation_guide.html["entscheiden, ob dieses Verfahren angewendet werden soll"].
