...
summary: 'Node3 wird installiert und gebootet und die ONTAP Software installiert, wenn auf Controller aktualisiert wird, die in ONTAP 9.15.1 oder höher eingeführt wurden, indem  `system controller replace` Befehle verwendet werden.' 
---
...
. Geben Sie  `Ctrl-C` während des automatischen Startvorgangs ein, um den Node am  `LOADER>`-Prompt anzuhalten.
...
Nach der Installation und dem Start von Node3, link:verify_node3_installation.html["Überprüfen Sie die Installation von node3."].
