...
summary: 'Node4 wird installiert und gestartet, und die ONTAP Software wird installiert, wenn auf Controller aktualisiert wird, die in ONTAP 9.15.1 oder höher eingeführt wurden, mithilfe von  `system controller replace` Befehlen.' 
---
...
+
Verkabeln Sie die folgenden Verbindungen und befolgen Sie dabei die Anweisungen in den _Installations- und Einrichtungsanweisungen_ für die Node4-Plattform, das entsprechende Disk-Shelf-Dokument und die _HA-Paarverwaltung_.
...
. [[auto_check_4_step11]]Wenn Knoten4 beim Startmenü stoppt, weisen Sie die Knoten2-Festplatten Knoten4 neu zu, indem Sie den folgenden Befehl auf Knoten4 ausführen:
+
...
NOTE: Im obigen Beispiel der Konsolenausgabe fordert ONTAP Sie zur Eingabe des Partner-Node-Namens auf, wenn das System Advanced Disk Partitioning (ADP)-Festplatten verwendet.

. [[auto_check_4_step27]]Starten Sie den Knoten an der LOADER-Eingabeaufforderung:
+
`boot_ontap menu`
...
Nach der Installation und dem Start von Node4, link:verify_node4_installation.html["Überprüfen Sie die Installation von Node4."].
