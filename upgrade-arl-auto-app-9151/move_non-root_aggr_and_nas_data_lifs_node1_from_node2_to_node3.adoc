...
summary: 'Es ist zu überprüfen, dass die node1 NAS-Daten-LIFs auf node2 von node2 auf node3 verschoben werden, wenn auf Controller aktualisiert wird, die in ONTAP 9.15.1 oder später eingeführt wurden, indem  `system controller replace` Befehle verwendet werden.' 
---
...
.Über diese Aufgabe
Remote LIFs verarbeiten den Datenverkehr zu den SAN LUNs während des Upgrade-Vorgangs. Das Verschieben von SAN LIFs ist für die Cluster- oder Service-Integrität während des Upgrades nicht erforderlich. SAN LIFs werden nicht verschoben, es sei denn, sie müssen neuen Ports zugeordnet werden. Sie überprüfen, ob die LIFs funktionsfähig sind und sich an den richtigen Ports befinden, nachdem Sie node3 in Betrieb genommen haben.
...
Nach dem Verschieben von Nicht-Root-Aggregaten und NAS-Daten-LIFs, die sich im Besitz von node1 befinden, von node2 zu node3, link:relocate_non_root_aggr_nas_lifs_from_node2_to_node3.html["Nicht-Root-Aggregate und NAS-Daten-LIFs von node2 auf node3 verschieben"].
