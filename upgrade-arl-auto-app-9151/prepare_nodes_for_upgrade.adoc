...
summary: 'Die Knoten werden vorbereitet und eine Reihe von Vorabprüfungen durchgeführt, wenn auf Controller aktualisiert wird, die in ONTAP 9.15.1 oder später eingeführt wurden, indem  `system controller replace` Befehle verwendet werden.' 
---
...
[role="lead"]
Der Austausch des Controllers beginnt mit einer Reihe von Vorprüfungen. Sie erfassen außerdem Informationen über die ursprünglichen Nodes, die später im Verfahren verwendet werden. Wenn Vorprüfungen fehlschlagen, müssen Sie die Aggregatzugehörigkeit korrigieren, bevor Sie mit dem Upgrade fortfahren können.



[[step-1-prepare-node1-and-node2]]
== Schritt 1: Knoten 1 und Knoten 2 vorbereiten

Der Controller-Austauschprozess wird gestartet und Vorabprüfungen auf node1 und node2 werden durchgeführt. Falls erforderlich, wird der Typ der verwendeten Self-Encrypting Drives ermittelt.
...
[NOTE]
====
Sie können den Befehl nur auf der erweiterten Berechtigungsebene ausführen `system controller replace start`:
...
....
. Wählen Sie  `y`, dann wird folgende Ausgabe angezeigt:
+
....
...
| AutoSupport Statusprüfung | Wenn AutoSupport-Benachrichtigungen nicht konfiguriert sind, schlägt die Aufgabe fehl. Bevor Sie mit dem Austausch des Controllers beginnen, müssen Sie AutoSupport aktivieren. 
...
== Schritt 3: node3 und node4 vorbereiten

Prüfen, ob auf node3 und node4 ein NODAR (keine Daten im Ruhezustand Verschlüsselung) Image installiert ist. Wenn es sich um ein NODAR Image handelt, ist ein DARE (Daten im Ruhezustand Verschlüsselung) Image zu installieren.

.Über diese Aufgabe
Führen Sie die folgenden Schritte auf node3 und node4 durch.
...
----
. Systemshell eingeben:
+
[source, cli]
----
systemshell
----
. [[check-build]]Prüfen, ob es sich bei dem Build um ein NODAR-Image handelt:
+
[source, cli]
----
cat /BUILD
----
+
In der Befehlsausgabe prüfen, ob  `build-type` ein NODAR-Image ist, zum Beispiel  `DEBUG SIM NODAR`.

+

NOTE: Wenn  `build-type` kein NODAR-Image ist, ist das Build ein DARE-Image.

. Handelt es sich bei dem Build um ein NODAR Image, ist ein DARE Image zu installieren.
+
.. Systemshell und das Boot-Menü verlassen:
+
[source, cli]
----
halt
----
.. Ein DARE-Image installieren. Weitere Informationen enthält der Knowledge Base Artikel link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/Netbooting_a_FAS_and_AFF_Controller_in_ONTAP_9["Netzwerkstart eines FAS und AFF Controllers in ONTAP 9"^].
