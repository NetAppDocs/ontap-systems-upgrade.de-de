...
summary: 'Nicht-Root-Aggregate und NAS-Daten-LIFs von node1 auf node2 verschieben und die Ressourcen von node1 auf node3 verschieben, wenn auf Controller aktualisiert wird, die in ONTAP 9.15.1 oder später eingeführt wurden, mithilfe von  `system controller replace` Befehlen.' 
---
...
Nach der Verschiebung der Nicht-Root-Aggregate und der NAS-Daten-LIFs, die sich im Besitz von node1 befinden, zu node2, link:relocate-failed-or-vetoed-aggr-node2.html["Fehlgeschlagene oder abgelehnte Aggregate auf node2 verschieben"].
