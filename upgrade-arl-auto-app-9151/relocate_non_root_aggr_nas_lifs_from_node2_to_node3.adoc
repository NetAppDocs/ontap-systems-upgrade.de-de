...
summary: 'Die Nicht-Root-Aggregate von Node2 werden beim Upgrade auf Controller, die in ONTAP 9.15.1 oder höher eingeführt wurden, mithilfe von  `system controller replace` Befehlen auf Node3 verschoben.' 
---
...
Nach der Verschiebung der Nicht-Root-Aggregate und der NAS-Daten-LIFs von Node2 auf Node3, link:relocate-failed-or-vetoed-aggr-node3.html["Fehlgeschlagene oder abgelehnte Aggregate auf node3 verschieben"].
