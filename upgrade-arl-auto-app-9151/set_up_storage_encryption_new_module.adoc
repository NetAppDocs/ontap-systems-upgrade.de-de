...
summary: 'Falls erforderlich, werden Storage Encryption konfiguriert, SSL-Zertifikate installiert und Key-Management-Server eingerichtet, wenn auf Controller aktualisiert wird, die in ONTAP 9.15.1 oder später eingeführt wurden, mithilfe von  `system controller replace` Befehlen.' 
---
...
Nach der Einrichtung der Speicherverschlüsselung auf dem neuen Controller-Modul, link:set_up_netapp_volume_encryption_new_module.html["Richten Sie die NetApp Volume- oder Aggregatverschlüsselung auf dem neuen Controller-Modul ein"].
