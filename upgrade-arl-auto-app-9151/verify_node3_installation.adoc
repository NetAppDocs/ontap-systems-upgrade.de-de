...
summary: 'Es sollte überprüft werden, ob die physischen Ports von node1 korrekt node3 zugeordnet sind, und die Netzwerkkonfiguration auf node3 wiederhergestellt werden, wenn beim Upgrade auf Controller, die in ONTAP 9.15.1 oder höher eingeführt wurden,  `system controller replace` Befehle verwendet werden.' 
---
...
[role="lead"]
Sie müssen überprüfen, ob die physischen Ports von node1 korrekt den physischen Ports von node3 zugeordnet sind. Dadurch kann node3 nach dem Upgrade mit anderen Nodes im Cluster und mit dem Netzwerk kommunizieren.

.Über diese Aufgabe
Siehe link:other-references.html["Quellen"] für den Link zum _Hardware Universe_, um Informationen zu den Ports der neuen Knoten zu erfassen. Diese Informationen werden später in diesem Abschnitt verwendet.

Die physische Portbelegung kann je nach Knotenmodell variieren. Wenn der neue Node hochfährt, versucht ONTAP zu ermitteln, welche Ports Cluster-LIFs hosten sollen, um automatisch ein Quorum zu erreichen.

Falls die physischen Ports auf node1 nicht direkt den physischen Ports auf node3 zugeordnet werden können, muss der nachfolgende Abschnitt <<restore_network_configuration_on_node3_9151,Stellen Sie die Netzwerkkonfiguration auf node3 wieder her>> verwendet werden, um die Netzwerkverbindung zu reparieren.
...
An diesem Punkt des Verfahrens wird die Operation unterbrochen, da node3 dem Quorum beitritt.



[[step-1-verify-that-node3-is-in-quorum]]
== Schritt 1: Überprüfen Sie, ob node3 im Quorum ist.

Überprüfen Sie, ob node3 im Quorum ist. Setzen Sie anschließend den Controller-Austausch fort.
...
. Der Austausch des Controllers wird aufgrund eines Eingriffs mit folgender Meldung unterbrochen:
+
....
...
== Schritt 2: Netzwerkkonfiguration auf node3 wiederherstellen
...
Weitere Informationen zum Erstellen und Neuerstellen von VLANs, Schnittstellengruppen und Broadcast-Domänen sind in link:other-references.html["Quellen"] zu _Network Management_ zu finden.
...
+
Während der Erreichbarkeitsreparatur versucht ONTAP, die Ports den korrekten Broadcast-Domänen zuzuordnen. Kann ONTAP die Erreichbarkeit eines Ports nicht ermitteln und gehört der Port keiner der bestehenden Broadcast-Domänen an, erstellt ONTAP neue Broadcast-Domänen für diese Ports.
...
Nach Überprüfung der Node3-Installation, link:restore_key-manager_configuration_node3.html["Schlüsselverwaltungskonfiguration auf Node-Name 3 wiederherstellen"].
