...
summary: 'Es sollte überprüft werden, ob die physischen Ports von node2 korrekt node4 zugeordnet sind, und die Netzwerkkonfiguration auf node4 wiederhergestellt werden, wenn auf Controller aktualisiert wird, die in ONTAP 9.15.1 oder später eingeführt wurden, indem  `system controller replace` Befehle verwendet werden.' 
---
...
[role="lead"]
Sie müssen überprüfen, ob die physischen Ports von node2 korrekt den physischen Ports von node4 zugeordnet sind. Dadurch kann node4 nach dem Upgrade mit anderen Nodes im Cluster und mit dem Netzwerk kommunizieren.
...
Falls die physischen Ports auf Knoten 2 nicht direkt den physischen Ports auf Knoten 4 zugeordnet werden können, muss der nachfolgende Abschnitt <<restore_network_configuration_on_node4_9151,Stellen Sie die Netzwerkkonfiguration auf node4 wieder her>> verwendet werden, um die Netzwerkverbindung wiederherzustellen.
...
An diesem Punkt des Verfahrens wird die Operation unterbrochen, da node4 dem Quorum beitritt.



[[step-1-verify-that-node4-is-in-quorum]]
== Schritt 1: Überprüfen Sie, ob node4 im Quorum ist.

Überprüfen Sie, ob node4 im Quorum ist. Setzen Sie anschließend den Controller-Austausch fort.
...
== Schritt 2: Netzwerkkonfiguration auf node4 wiederherstellen
...
Nach Überprüfung der Node4-Installation, link:restore_key-manager_config_node4.html["Schlüsselverwaltungskonfiguration auf Node-Name 4 wiederherstellen"].
