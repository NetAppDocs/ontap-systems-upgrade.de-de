...
* Wenn Sie von einem AFF A320 System aktualisieren, können Volume-Verschiebungen zur Aktualisierung der Controller-Hardware genutzt werden oder der technische Support kontaktiert werden. Siehe link:other-references.html["Quellen"] für einen Link zu _Upgrade durch Verschieben von Volumes oder Speicher_.
...
Nachdem Sie sich über dieses ARL-Verfahren informiert haben, link:decide_to_use_the_aggregate_relocation_guide.html["entscheiden, ob dieses ARL-Verfahren angewendet werden soll"].
