...
* Wenn Sie ein System mit Speichermedien aufrüsten, müssen Sie diesen gesamten Abschnitt ausfüllen und anschließend zu den link:set_fc_or_uta_uta2_config_on_node3.html#configure-fc-ports-on-node3["Konfigurieren Sie FC-Ports auf node3"] und link:set_fc_or_uta_uta2_config_on_node3.html#check-config-uta-uta2-node3["UTA/UTA2-Ports in node3 prüfen und konfigurieren"] Abschnitten übergehen, indem Sie Befehle an der Cluster-Eingabeaufforderung eingeben.
...
+
Siehe link:other-references.html["Quellen"] zum Verweis auf die _HA-Paarverwaltung_.
...
+
Geben Sie die Passphrase und die Sicherungsinformationen ein, die Sie zuvor im Verfahren notiert haben. Siehe link:complete-preparation-for-upgrade.html#manage_storage_encryption_using_okm["Management der Storage-Verschlüsselung mit dem Onboard Key Manager"].
...
* Wenn Sie ein System mit einer FC- oder UTA/UTA2-Konfiguration haben,link:set_fc_or_uta_uta2_config_on_node3.html["Legen Sie die FC- oder UTA/UTA2-Ports auf Knoten3 fest und konfigurieren Sie sie"] .
* Wenn Sie keine FC- oder UTA/UTA2-Konfiguration haben,link:reassign-node1-disks-to-node3.html#reassign-node1-node3-app-step1["Neuzuweisung der Node1-Festplatten zu Node3, Schritt 1"] damit Knoten3 die Festplatten von Knoten1 erkennen kann.
* Wenn Sie eine MetroCluster Konfiguration haben,link:set_fc_or_uta_uta2_config_on_node3.html["Legen Sie die FC- oder UTA/UTA2-Ports auf Knoten3 fest und konfigurieren Sie sie"] um die an den Knoten angeschlossenen Festplatten zu erkennen.
