...
Falls die physischen Ports auf node1 nicht direkt den physischen Ports auf node3 zugeordnet werden können, muss der nachfolgende Abschnitt <<restore_network_configuration_on_node3,Stellen Sie die Netzwerkkonfiguration auf node3 wieder her>> verwendet werden, um die Netzwerkverbindung zu reparieren.
...
+
Während der Erreichbarkeitsreparatur versucht ONTAP, die Ports den korrekten Broadcast-Domänen zuzuordnen. Kann ONTAP die Erreichbarkeit eines Ports nicht ermitteln und gehört dieser keiner der bestehenden Broadcast-Domänen an, erstellt ONTAP neue Broadcast-Domänen für diese Ports.
...
Nach Überprüfung der Node3-Installation, link:restore_key-manager_configuration_node3.html["Die Key-Manager-Konfiguration auf node3 wiederherstellen"].
