...
Falls die physischen Ports auf Node2 nicht direkt den physischen Ports auf Node4 zugeordnet werden können, muss der nachfolgende Abschnitt <<restore_network_configuration_on_node4,Stellen Sie die Netzwerkkonfiguration auf node4 wieder her>> verwendet werden, um die Netzwerkverbindung wiederherzustellen.
...
Nach Überprüfung der Node4-Installation, link:restore_key-manager_config_node4.html["Die Key-Manager-Konfiguration auf node4 wiederherstellen"].
