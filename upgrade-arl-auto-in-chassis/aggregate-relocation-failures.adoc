...
+
Für detaillierte Informationen zu den  `storage aggregate relocation start`, `override-vetoes` und  `override-destination-checks` Optionen verweisen Sie auf link:other-references.html["Quellen"] zur Verlinkung auf die _ONTAP 9 Command reference_.
...
+
Wenn node2 ersetzt wird, kommt aggr_node_1 mit node1 als seinem Home-Node statt node2 online.
...
+
Um Aggregate zu identifizieren, die nicht korrekt verschoben wurden, konsultieren Sie die Liste der Aggregate mit dem Home Owner von node1, die Sie im Abschnitt link:prepare-nodes-for-upgrade.html["Bereiten Sie die Knoten für ein Upgrade vor"] erhalten haben, und vergleichen Sie diese mit der Ausgabe des obigen Befehls.

. Vergleichen Sie die Ausgabe von Schritt 1 mit der Ausgabe, die Sie für node1 im Abschnitt link:prepare-nodes-for-upgrade.html["Bereiten Sie die Knoten für ein Upgrade vor"] erfasst haben, und notieren Sie alle Aggregate, die nicht korrekt verschoben wurden.
