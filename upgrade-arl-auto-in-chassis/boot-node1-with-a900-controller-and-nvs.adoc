...
[role="lead"]
Node1 mit den Ersatzmodulen ist jetzt bereit zum Booten. Die unterstützten Ersatzmodule sind in der link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["Unterstützte Systeme Matrix"] aufgeführt.
...
+
.. Antwort `y` zur Bestätigungsaufforderung für gemischte Plattformen.
...
+
Geben Sie die Passphrase und die Sicherungsinformationen ein, die Sie zuvor im Verfahren notiert haben. Siehe link:complete-preparation-for-upgrade.html#manage-storage-encryption-using-okm["Management der Storage-Verschlüsselung mit dem Onboard Key Manager"].
...
. Wenn der Knoten beim Startmenü stoppt, weisen Sie die alten Knoten1-Festplatten dem Ersatzknoten1 neu zu, indem Sie den folgenden Befehl auf Knoten1 ausführen:
+
...
Nach dem Hochfahren von node1 mit den Ersatzsystemmodulen, link:verify-node1-installation.html["Überprüfen Sie die Installation von node1"].
