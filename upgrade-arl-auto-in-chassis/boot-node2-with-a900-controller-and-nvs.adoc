...
[role="lead"]
Node2 mit den Ersatzmodulen ist nun startbereit. Die unterstützten Ersatzmodule sind in der link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["Unterstützte Systeme Matrix"] aufgeführt.
...
. Wenn der Knoten beim Startmenü stoppt, weisen Sie die alten Knoten2-Festplatten dem Ersatzknoten2 neu zu, indem Sie den folgenden Befehl auf Knoten2 ausführen:
+
...
Nach dem Hochfahren von Node2 mit den Ersatzsystemmodulen, link:verify-node2-installation.html["Überprüfen Sie die Installation von node2."].
