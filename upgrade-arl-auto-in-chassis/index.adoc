...
[role="lead"]
Es gibt mehrere Methoden zur Aggregate-Umverlagerung (ARL) für die Aktualisierung der Controller-Hardware. Dieses Verfahren beschreibt, wie Speichercontroller in einem HA-Paar aktualisiert werden, indem das bestehende System in das Ersatzsystem umgewandelt wird, wobei das bestehende Systemgehäuse, die Festplatten und die darauf gespeicherten Daten erhalten bleiben.
...
NOTE: Die Bezeichnungen *node1* und *node2* werden in diesem Dokument nur als Referenz für Knotennamen verwendet. Wenn Sie dem Verfahren folgen, müssen Sie die tatsächlichen Namen Ihrer Knoten verwenden.
...
Sie müssen wichtige Faktoren berücksichtigen, bevor Sie mit dem Upgrade beginnen.


NOTE: Sie müssen alle folgenden wichtigen Informationen überprüfen.

* Dieses Verfahren ist komplex und setzt fortgeschrittene ONTAP Administrationskenntnisse voraus. Sie sollten die link:guidelines-for-upgrading-controllers-with-arl.html["Richtlinien zum Upgraden von Controllern"] und link:overview-of-the-arl-upgrade.html["ARL-Upgradesequenz"] sorgfältig lesen und verstehen, bevor Sie mit dem Upgrade beginnen.
...
* Falls Sie einen Switch besitzen, der von der ONTAP Version und dem Ersatzsystem, auf das Sie aktualisieren, nicht unterstützt wird, lesen Sie link:other-references.html["Quellen"], um zum _Hardware Universe_ zu gelangen.
...
* Wenn Sie ein Upgrade von einem bestehenden System mit Ausfallzeit durchführen, die im link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["Unterstützte Systeme Matrix"] enthalten ist, können Sie die Controller-Hardware aktualisieren, indem Sie Speicher verschieben oder den technischen Support kontaktieren. Siehe link:other-references.html["Quellen"], um auf _Upgrade durch Verschieben von Volumes oder Speicher_ zu verlinken.




[[automated-controller-upgrade-process]]
== Automatisierter Controller-Upgrade-Prozess
...
Nachdem ich von diesem ARL-Upgrade-Verfahren erfahren hatte, link:decide-to-use-the-aggregate-relocation-guide.html["entscheiden, ob dieses Verfahren angewendet werden soll"].
