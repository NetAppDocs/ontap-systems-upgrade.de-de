...
Es ist nicht möglich, die Version von ONTAP auf dem Bootmedium des Ersatz-Controllermoduls zu überprüfen, es sei denn, es ist in einem Gehäuse installiert und eingeschaltet. Die ONTAP Version auf dem Bootmedium des Ersatzsystems muss mit der ONTAP Version des alten Systems, das Sie aktualisieren, übereinstimmen, und sowohl das primäre als auch das Backup-Bootimage auf dem Bootmedium müssen übereinstimmen. Um die minimal unterstützte ONTAP Version für Ihr Upgrade zu überprüfen, siehe link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["Unterstützte Systeme Matrix"].
...
* Laden Sie die benötigten Systemdateien für Ihr System und die richtige Version von ONTAP von der _NetApp Support Site_ herunter. Siehe link:other-references.html["Quellen"] zum Verlinken auf die _NetApp Support Site_.
...
.Schritte
. Siehe link:other-references.html["Quellen"] zum Verlinken auf die _NetApp Support Site_, um die für den Netboot des Systems verwendeten Dateien herunterzuladen.
...
+
.. Führen Sie an der folgenden Eingabeaufforderung den  `wipeconfig` Befehl aus. Drücken Sie anschließend die Eingabetaste:
+
[listing]
----
Please choose one of the following:
...
Nach dem Netzwerkstart von node1, link:cable-node1-for-shared-cluster-HA-storage.html["Kabel node1 für gemeinsam genutztes Cluster-HA und Storage"].
