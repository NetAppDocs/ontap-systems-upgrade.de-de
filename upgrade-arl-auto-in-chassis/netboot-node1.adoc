...
Die auf dem Bootmedium des Ersatz-Controllermoduls installierte ONTAP -Version kann nur überprüft werden, wenn das Modul in einem Gehäuse installiert und eingeschaltet ist.  Die ONTAP Version auf dem Ersatzsystem-Bootmedium muss mit der ONTAP Version auf dem alten System, das Sie aktualisieren, übereinstimmen, und sowohl das primäre als auch das Backup-Bootimage auf dem Bootmedium müssen übereinstimmen.  Um die minimal unterstützte ONTAP Version für Ihr Upgrade zu überprüfen, siehelink:decide_to_use_the_aggregate_relocation_guide.html#supported-systems-in-chassis["Unterstützte Systeme Matrix"] Die
