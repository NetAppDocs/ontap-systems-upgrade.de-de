...
summary: Wenn ARL verwendet wird, um Controller-Modelle im selben Chassis zu aktualisieren, muss node2 nach dem Austausch des entsprechenden Ersatzsystemmoduls per Netboot gestartet werden. 
---
= Netzboot Nr. 2
:hardbreaks:
:allow-uri-read: 
...
Nach dem Netzwerkstart von node2, link:cable-node2-for-shared-cluster-HA-storage.html["Kabelknoten 2 für gemeinsam genutztes Cluster-HA und Speicher"].
