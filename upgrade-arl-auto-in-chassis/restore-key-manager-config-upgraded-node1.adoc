...
Nach der Wiederherstellung der Key-Manager-Konfiguration auf dem aktualisierten node1, link:move-non-root-aggr-nas-lifs-node1-from-node2-to-upgraded-node1.html["Verschieben Sie die Nicht-Root-Aggregate von node1 und die NAS-Daten-LIFs von node2 auf den aktualisierten node1."].
