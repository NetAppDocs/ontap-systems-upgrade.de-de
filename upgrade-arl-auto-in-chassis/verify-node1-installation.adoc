...
== Schritt 1: Überprüfen Sie, ob node1 im Quorum ist

Überprüfen Sie, ob node1 im Quorum ist. Setzen Sie anschließend den Controller-Austausch fort.
...
. Während der Austausch des Controllers pausiert ist, fahren Sie mit <<restore_network_configuration_on_node1,Stellen Sie die Netzwerkkonfiguration auf node1 wieder her>> fort.




== Schritt 2: Netzwerkkonfiguration auf node1 wiederherstellen
...
Nach Überprüfung der Installation von node1, link:restore-key-manager-config-upgraded-node1.html["Stellen Sie die Key-Manager-Konfiguration auf dem aktualisierten node1 wieder her"].
