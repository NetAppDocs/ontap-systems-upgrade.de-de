...
.Über diese Aufgabe
Nachdem Sie node2 mit dem Ersatz-Systemmodul gestartet haben, wird überprüft, ob es korrekt installiert ist. Es ist erforderlich, zu warten, bis node2 dem Quorum beigetreten ist, und anschließend den Controller Replacement-Vorgang fortzusetzen.
...
== Schritt 1: Überprüfen Sie, ob node2 im Quorum ist.

Überprüfen Sie, ob node2 im Quorum ist. Setzen Sie anschließend den Controller-Austausch fort.
...
. Während der Austausch des Controllers pausiert ist, fahren Sie mit <<restore_network_configuration_on_node2,Stellen Sie die Netzwerkkonfiguration auf node2 wieder her>> fort.
...
== Schritt 2: Netzwerkkonfiguration auf node2 wiederherstellen
...
.Über diese Aufgabe
Weitere Informationen zum Erstellen und Neuerstellen von VLANs, Schnittstellengruppen und Broadcast-Domänen finden Sie unter link:other-references.html["Quellen"] zum Inhalt _Network Management_.
...
Nach Überprüfung der Node2-Installation, link:restore-key-manager-config-node2.html["Schlüsselverwaltungskonfiguration auf node2 wiederherstellen"].
