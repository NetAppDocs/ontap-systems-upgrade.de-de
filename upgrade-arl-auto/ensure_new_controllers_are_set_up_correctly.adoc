...
. Falls in der Konfiguration Self-Encrypting Drives verwendet werden und die festgelegt sind `kmip.init.maxwait` Variabel auf `off` (Beispiel in link:install_boot_node4.html#step27["installieren und booten sie node4, Schritt 27"]), Sie müssen die Einstellung der Variable aufheben:
+
