...
[role="lead"]
Sie müssen überprüfen, ob die physischen Ports auf node1 korrekt den physischen Ports auf node3 zugeordnet sind. Dadurch kann node3 nach dem Upgrade mit anderen Knoten im Cluster und mit dem Netzwerk kommunizieren.
...
Die Porteinstellungen können je nach Knotenmodell variieren. Sie müssen die Port- und LIF-Konfiguration des ursprünglichen Knotens mit der geplanten Nutzung und Konfiguration des neuen Knotens kompatibel machen. Dies liegt daran, dass der neue Knoten beim Start dieselbe Konfiguration abruft, was bedeutet, dass ONTAP beim Starten von node3 versucht, LIFs auf denselben Ports zu hosten, die auf node1 verwendet wurden.

Wenn die physischen Ports von node1 nicht direkt den physischen Ports von node3 zugeordnet sind, sind daher Softwarekonfigurationsänderungen erforderlich, um die Cluster-, Management- und Netzwerkverbindung nach dem Neustart wiederherzustellen. Wenn die Cluster-Ports von node1 ebenfalls nicht direkt den Cluster-Ports von node3 zugeordnet sind, tritt node3 nach einem Neustart möglicherweise nicht automatisch wieder dem Quorum bei, bis die Softwarekonfiguration so geändert wird, dass die Cluster-LIFs auf den korrekten physischen Ports gehostet werden.
...
+
Weitere Informationen finden Sie unter link:other-references.html["Quellen"] dem Link _Network Management_ oder der _ONTAP 9 Command reference_.
