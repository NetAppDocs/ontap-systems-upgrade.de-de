...
[role="lead"]
Sie müssen überprüfen, ob die physischen Ports auf node2 korrekt den physischen Ports auf node4 zugeordnet sind. Dadurch kann node4 nach dem Upgrade mit anderen Knoten im Cluster und mit dem Netzwerk kommunizieren.
...
Wenn die physischen Ports von node2 nicht direkt den physischen Ports von node4 zugeordnet sind, sind daher Softwarekonfigurationsänderungen erforderlich, um die Cluster-, Management- und Netzwerkverbindung nach dem Booten wiederherzustellen. Wenn die Cluster-Ports von node2 ebenfalls nicht direkt den Cluster-Ports von node4 zugeordnet sind, tritt node4 nach einem Neustart möglicherweise nicht automatisch wieder dem Quorum bei, bis eine Softwarekonfigurationsänderung vorgenommen wurde, um die Cluster-LIFs auf den korrekten physischen Ports zu hosten.
...
+
Siehe link:other-references.html["Quellen"], um zu _Netzwerkmanagement_ oder der _ONTAP 9 Command reference_ zu verlinken, und siehe _Konfigurieren von Failover-Einstellungen auf einem LIF_ für weitere Informationen.
