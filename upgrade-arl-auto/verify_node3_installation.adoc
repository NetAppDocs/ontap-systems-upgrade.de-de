...
Wenn Sie NetApp Aggregate Encryption (NAE) oder NetApp Volume Encryption (NVE) zur Verschlüsselung von Volumen auf dem zu aktualisierenden System verwenden, muss die Verschlüsselungskonfiguration mit den neuen Knoten synchronisiert werden. Wenn Sie den Schlüsselmanager nicht wiederherstellen, werden beim Verschieben der node1-Aggregate mithilfe von ARL von node2 auf node3 die verschlüsselten Volumen offline genommen.
