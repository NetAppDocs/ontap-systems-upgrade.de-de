...
CAUTION: In dieser Phase befindet sich die MetroCluster-Konfiguration nicht im Normalzustand und es können Fehler auftreten, die behoben werden müssen. Siehe link:verify_health_of_metrocluster_config.html["Überprüfen Sie den Systemzustand der MetroCluster-Konfiguration"].
...
.Über diese Aufgabe
Weitere Informationen zum Erstellen und Neuerstellen von VLANs, Schnittstellengruppen und Broadcast-Domänen sind unter link:other-references.html["Quellen"]_Netzwerkverwaltung_ verfügbar.
...
Wenn Sie NetApp Aggregate Encryption (NAE) oder NetApp Volume Encryption (NVE) zur Verschlüsselung von Volumen auf dem zu aktualisierenden System verwenden, muss die Verschlüsselungskonfiguration mit den neuen Knoten synchronisiert werden. Wenn Sie den Key-Manager nicht wiederherstellen, werden beim Verschieben der Aggregate von Knoten2 mithilfe von ARL von Knoten3 auf Knoten4 die verschlüsselten Volumen offline genommen.
