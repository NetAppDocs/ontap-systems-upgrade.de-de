...
CAUTION: Zu diesem Zeitpunkt befindet sich die MetroCluster Konfiguration nicht im Normalzustand und es müssen möglicherweise Fehler behoben werden. Sehenlink:verify_health_of_metrocluster_config.html["Überprüfen Sie den Systemzustand der MetroCluster-Konfiguration"] .
