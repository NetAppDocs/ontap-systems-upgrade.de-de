...
NOTE: Bevor Sie fortfahren, müssen Sie die Aggregate offline nehmen und anschließend die Aggregate auf den internen Festplatten zerstören. Weitere Informationen zur Verwaltung von Aggregaten enthält link:other-references.html["Quellen"] der Abschnitt _Festplatten- und Aggregatverwaltung über die Befehlszeile_.
...
NOTE: Bevor Sie fortfahren, müssen Sie die Volumes auf ein externes Festplattengehäuse verschieben, die Aggregate offline nehmen und anschließend die Aggregate auf den internen Festplatten zerstören. Weitere Informationen zum Verschieben von Volumes enthält link:other-references.html["Quellen"] der Abschnitt _Festplatten- und Aggregatverwaltung mit der CLI_.



| Enthalten Root-Volumes auf den internen Laufwerken | Fahren Sie mit diesem Verfahren nicht fort. Die Controller können aktualisiert werden, indem auf link:other-references.html["Quellen"] zum _NetApp Support Site_ verwiesen und das Verfahren _Upgrading the controller hardware on a pair of nodes running clustered Data ONTAP by moving volumes_ verwendet wird. 


| Enthalten nicht-Root-Volumes auf den internen Laufwerken und Sie können die Volumes nicht in einen externen Speicher verschieben | Fahren Sie mit diesem Verfahren nicht fort. Die Controller können mit dem Verfahren _Upgrading the controller hardware on a pair of nodes running clustered Data ONTAP by moving volumes_ aktualisiert werden. Siehe link:other-references.html["Quellen"] zum Verweis auf die _NetApp Support Site_, auf der dieses Verfahren verfügbar ist. 
|===


.Was kommt als Nächstes?
Nachdem festgestellt wurde, ob der Controller Aggregate auf internen Festplattenlaufwerken hat link:prepare_nodes_for_upgrade.html["Bereiten Sie die Knoten für das Upgrade vor"].
