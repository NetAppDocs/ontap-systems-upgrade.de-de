...
+
Die folgenden Anleitungen helfen Ihnen beim korrekten Anschließen der Kabel. Gehen Sie zu link:other-references.html["Quellen"] , um zu den Anleitungen zu gelangen.

+
** _Installations- und Einrichtungsanweisungen_ für die node3-Plattform
...
. Geben Sie an der Eingabeaufforderung für den Wartungsmodus den folgenden Befehl ein:
+
`halt`
...
| Dual-Chassis-Konfiguration (mit Controllern in anderem Chassis) | Gehe zu<<man_install3_step8,Schritt 8>> . 
...
.. Gehe zu<<man_install3_step8,Schritt 8>> .


|===
. [[man_install3_step8]]Konfigurieren Sie node3 für ONTAP:
+
`set-defaults`
...
+
Geben Sie die Passphrase und die Sicherungsinformationen ein, die Sie zuvor im Verfahren notiert haben. Siehe link:manage_authentication_okm.html["Verwaltung von Authentifizierungsschlüssel mit dem Onboard Key Manager"].
...
. Drücken Sie STRG-C, um das Bootmenü anzuzeigen.
...
Nach der Installation und dem Start von Node3, link:set_fc_uta_uta2_config_node3.html["Legen Sie die FC- oder UTA/UTA2-Konfiguration auf Knoten3 fest"].
