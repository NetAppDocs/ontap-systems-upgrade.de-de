...
[role="lead"]
Sie müssen sicherstellen, dass die physischen Ports auf Knoten2 korrekt den physischen Ports auf Knoten4 zugeordnet sind. Dadurch kann Knoten4 nach dem Upgrade mit anderen Knoten im Cluster und mit dem Netzwerk kommunizieren.

.Bevor Sie beginnen
Sie müssen bereits Informationen zu den Ports der neuen Knoten haben, um auf diese Informationen zuzugreifen, siehe link:other-references.html["Quellen"] zum Verweis auf das _Hardware Universe_. Diese Informationen werden später in diesem Abschnitt verwendet.
...
Nach der Portzuordnung von Knoten2 zu Knoten4, link:move_nas_lifs_node2_from_node3_node4_verify_san_lifs_node4.html["Verschieben Sie die NAS-Daten-LIFs, die sich im Besitz von node2 befinden, von node3 zu node4 und überprüfen Sie die SAN-LIFs auf node4."].
