...
+
Siehe link:other-references.html["Quellen"] zum Verlinken auf _HA pair management_ und den Abschnitt _Best practices for HA pairs_. Keiner der ursprünglichen Knoten sollte mit mehr als 50 Prozent Auslastung betrieben werden; wenn ein Knoten mit weniger als 50 Prozent Auslastung läuft, kann er die Lasten beider Knoten während des Controller-Upgrades übernehmen.
...
Informationen zum Entsperren der Benutzerkonten sind unter link:other-references.html["Quellen"] im Abschnitt zur _System Administration Reference_ zu finden.

====
.. Siehe link:other-references.html["Quellen"] für einen Link zur _NetApp Support Site_ und zum Herunterladen des Performance and Statistics Collector (Perfstat Converged).
+
...
. Siehe link:other-references.html["Quellen"] für einen Link zur _NetApp Support Site_ und zum Öffnen eines Supportfalls auf der NetApp Support Site.
+
...
+
Sie müssen node3 und node4 physisch überprüfen, um festzustellen, ob die NVMEM- oder NVRAM-Batterien geladen sind. Informationen zu den LEDs für das Modell von node3 und node4 finden sich unter link:other-references.html["Quellen"] zum _Hardware Universe_.
...
+
Auf den neuen Knoten muss dieselbe Version von ONTAP 9.x installiert sein wie auf den ursprünglichen Knoten. Falls auf den neuen Knoten eine andere Version von ONTAP installiert ist, müssen die neuen Controller nach der Installation per Netzwerkstart gebootet werden. Anweisungen zum Upgrade von ONTAP finden sich unter link:other-references.html["Quellen"] zum Link _Upgrade ONTAP_.
...
....
+
Befindet sich einer der Knoten in der Phase der teilweisen Rückgabe, kann der  `storage failover giveback` Befehl zur Durchführung der Rückgabe verwendet werden, anschließend stellt der  `storage failover show-giveback` Befehl sicher, dass keine Aggregate mehr zurückgegeben werden müssen. Detaillierte Informationen zu den Befehlen bietet link:other-references.html["Quellen"] der Link zu _HA pair management_.
...
+
Wenn weder node1 noch node2 Aggregate besitzt, für die sie der aktuelle Eigentümer (aber nicht der Home-Eigentümer) sind, gibt das System eine Meldung ähnlich dem folgenden Beispiel zurück:
...
....
+
Falls Festplatten fehlen, lesen Sie link:other-references.html["Quellen"], um auf _Festplatten- und Aggregatverwaltung mit der CLI_, _Logische Speicherverwaltung mit der CLI_ und _HA-Paarverwaltung_ zu verweisen, um den Speicher für das HA-Paar zu konfigurieren.
...
+
Für detaillierte Informationen zum  `network interface revert` Befehl verweisen Sie sich auf link:other-references.html["Quellen"] den Link zur _ONTAP 9 Command reference_.
...
+
Falls eine der Festplatten ausgefallen ist, entfernen Sie diese gemäß den Anweisungen unter _Festplatten- und Aggregatverwaltung mit der CLI_. (Siehe link:other-references.html["Quellen"] zum Verweis auf _Festplatten- und Aggregatverwaltung mit der CLI_.)
...
+
VLANs werden über physische Ports konfiguriert. Wenn sich die physischen Ports ändern, müssen die VLANs im weiteren Verlauf des Verfahrens neu erstellt werden.
...
+
Wenn weder iSCSI noch FC für die SVM konfiguriert ist, zeigt der Befehl eine Meldung ähnlich dem folgenden Beispiel an:
...
+
Wenn entweder iSCSI oder FC für die SVM konfiguriert ist, zeigt der Befehl eine Meldung ähnlich dem folgenden Beispiel an:
...
....
+
Knoten in einem Cluster befinden sich im Quorum, wenn die einfache Mehrheit der Knoten fehlerfrei funktioniert und miteinander kommunizieren kann. Weitere Informationen sind unter link:other-references.html["Quellen"] im _System Administration Reference_ zu finden.
...
+
Möglicherweise möchten Sie die Netzwerkparameter der Remote-Management-Geräte, in diesem Fall die SPs, vom ursprünglichen System für die SPs auf den neuen Knoten wiederverwenden. Detaillierte Informationen zu den SP finden Sie unter link:other-references.html["Quellen"] dem Link zur _System Administration Reference_ und der _ONTAP 9 Command reference_.
...
....
. Sie erhalten neue Lizenzschlüssel für die neuen Nodes auf der _NetApp Support Site_. Informationen zur Verknüpfung mit der _NetApp Support Site_ finden Sie unter link:other-references.html["Quellen"].
+
...
Aktivieren Sie AutoSupport gemäß den Anweisungen in der _System Administration Reference_. (Siehe link:other-references.html["Quellen"] für den Link zur _System Administration Reference_.)
...
+
Detaillierte Informationen zu AutoSupport finden Sie unter link:other-references.html["Quellen"] zum Verweis auf die _System Administration Reference_ und die _ONTAP 9 Command reference_.
...
Nachdem die Knoten für das Upgrade vorbereitet wurden, link:manage_authentication_okm.html["Verwalten Sie Authentifizierungsschlüssel mithilfe des Onboard Key Manager."].
