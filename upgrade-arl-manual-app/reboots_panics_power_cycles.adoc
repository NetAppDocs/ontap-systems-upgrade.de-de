...
Einige oder alle Aggregate wurden von node1 auf node2 verschoben, und HA ist weiterhin aktiviert. node2 übernimmt das Root-Volume von node1 sowie alle Nicht-Root-Aggregate, die nicht verschoben wurden.

.Über diese Aufgabe
Die Eigentumsverhältnisse der verschobenen Aggregate entsprechen denen der übernommenen Nicht-Root-Aggregate, da sich der Eigentümer nicht geändert hat. Wenn Knoten 1 den  `waiting for giveback state` betritt, gibt Knoten 2 alle Nicht-Root-Aggregate von Knoten 1 zurück.
...
.Über diese Aufgabe
Knoten 1 übernimmt alle Aggregate von Knoten 2 sowie alle eigenen Aggregate, die er zu Knoten 2 verschoben hatte. Wenn Knoten 2 in den  `Waiting for Giveback` Zustand wechselt, gibt Knoten 1 alle Aggregate von Knoten 2 zurück.
...
. Knoten 2 aufbring.
+
Während des Hochfahrens von node2 kommt es zu einem Clientausfall für alle Aggregate.
...
Knoten 3 übernimmt nach einem Absturz von Knoten 2 nicht, da HA bereits deaktiviert ist.
...
Knoten 2 hat einige oder alle seiner Aggregate auf Knoten 3 verschoben, der Daten von den verschobenen Aggregaten bereitstellt. HA ist deaktiviert.

.Über diese Aufgabe
Es gibt einen Kundenausfall bei Aggregaten, die nicht verlegt wurden.
...
Falls node3 abstürzt, während node2 Aggregate zu node3 verschiebt, bricht node2 die Verschiebung aller verbleibenden Aggregate ab.
...
. Knoten 3 aufbring.
+
Es liegt ein Clientausfall für alle Aggregate vor.
