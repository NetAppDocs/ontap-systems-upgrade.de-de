...
NOTE: Ihr System verfügt möglicherweise nicht über Intercluster-LIFs. Sie haben eine Cluster-Management-LIF nur auf einem Knoten eines Knotenpaares. Eine Cluster-Management-LIF wird in der Beispielausgabe von link:record_node1_information.html#step1["Schritt 1"] in _Record node1 port information_ angezeigt.
...
+
Die Broadcast-Domänen müssen im weiteren Verlauf des Verfahrens den Ports des neuen Controllers zugeordnet werden.
...
Nach der Aufzeichnung der node2-Informationen, link:retire_node2.html["Node2 außer Betrieb nehmen"].
