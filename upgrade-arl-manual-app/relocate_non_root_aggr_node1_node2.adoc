...
.Schritte
. [[relocate-step1]]Verschieben Sie die Nicht-Root-Aggregate, indem Sie die folgenden Teilschritte ausführen:
+
...
+
Die Ausgabe zeigt  `Done` für ein Aggregat an, nachdem es verschoben wurde.
...
Weitere Informationen zu Befehlen zur Speicheraggregatverschiebung finden Sie unter link:other_references.html["Quellen"] dem Link zum Abschnitt „Disk- und Aggregatverwaltung mit der CLI“ und der „ONTAP 9 Command reference“.
...
Nach dem Verschieben der Nicht-Root-Aggregate von node1 zu node2, link:move_nas_lifs_node1_node2.html["Verschieben Sie NAS-Daten-LIFs, die node1 gehören, nach node2"].
