...
Weitere Informationen zu den Befehlen für die Speicheraggregatverschiebung finden Sie unter link:other_references.html["Quellen"] _Disk and aggregate management with the CLI_ und der _ONTAP 9 Command reference_.
...
Nach der Verschiebung der Nicht-Root-Aggregate von Knoten2 zu Knoten3 link:move_nas_lifs_node2_node3.html["Verschieben Sie NAS-Daten-LIFs, die zu node2 gehören, zu node3"].
