...
+
Da in einem Cluster mit einer geraden Anzahl von Knoten ein Stimmengleichstand möglich ist, erhält ein Knoten ein zusätzliches, bruchstückhaftes Stimmgewicht, genannt Epsilon. Weitere Informationen finden sich in der link:other-references.html["Quellen"]Systemadministrationsreferenz.
...
WARNING: Wenn sich Knoten1 im selben Gehäuse wie Knoten2 befindet, schalten Sie das Gehäuse nicht über den Netzschalter oder durch Ziehen des Netzkabels aus.  Wenn Sie dies tun, wird Knoten2, der Daten bereitstellt, ausfallen.
...
. Optional kann node1 nach Abschluss des Upgrades außer Betrieb genommen werden. Siehe link:decommission_old_system.html["Ausmustern des alten Systems"].


.Was kommt als Nächstes?
Nach der Außerbetriebnahme von node1, link:install_boot_node3.html["Node3 installieren und booten"].
