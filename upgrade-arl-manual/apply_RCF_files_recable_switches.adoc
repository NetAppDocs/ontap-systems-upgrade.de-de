...
.Bevor Sie beginnen
Die Nutzung der FC-Switch-Ports muss der im Inhalt _Fabric-attached MetroCluster Installation and Configuration_ beschriebenen Nutzung entsprechen, damit die RCFs verwendet werden können. Zu link:other-references.html["Quellen"] gelangen, um auf den Inhalt _Fabric-attached MetroCluster Installation and Configuration_ zu verlinken.
