...
+
Sie können die Ausgabe mit der Ausgabe vergleichen, die Sie inlink:prepare_nodes_for_upgrade.html#man_prepare_nodes_step29["Schritt 29"] von _Knoten für das Upgrade vorbereiten_.
...
+
Gehen Sie zu link:other-references.html["Quellen"], um auf die _System Administration Reference_ für Informationen zu den SPs und auf die _ONTAP 9 Command reference_ für detaillierte Informationen zum  `system service- processor network modify` Befehl zu verlinken.

. Wenn Sie ein Cluster ohne Switches auf den neuen Knoten einrichten möchten, gehen Sie zu link:other-references.html["Quellen"] zum Link zur _Network Support Site_ und befolgen Sie die Anweisungen in _Transitioning to a two-node switchless cluster_.
