...
. [[man_install3_steep16]]Festlegen des `bootarg.storageencryption.support` Und `kmip.init.maxwait` Variablen, um eine Boot-Schleife zu vermeiden, nachdem die node1-Konfiguration geladen wurde.
+
...
** Sobald das Controller-Upgrade auf dem HA-Paar abgeschlossen ist, müssen Sie den festlegen `kmip.init.maxwait` Variabel. Siehe link:ensure_controllers_set_up_correctly.html["Vergewissern Sie sich, dass die neuen Controller ordnungsgemäß eingerichtet sind"].
