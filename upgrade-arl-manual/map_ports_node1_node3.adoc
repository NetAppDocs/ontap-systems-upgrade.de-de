...
Sie müssen die Port- und LIF-Konfiguration des ursprünglichen Knotens mit der geplanten Konfiguration des neuen Knotens kompatibel machen. Dies liegt daran, dass der neue Knoten beim Start dieselbe Konfiguration erneut abruft, was bedeutet, dass beim Booten von node3 ONTAP versucht, LIFs auf denselben Ports zu hosten, die auf node1 verwendet wurden.

Wenn die physischen Ports von node1 nicht direkt den physischen Ports von node3 zugeordnet sind, sind daher Softwarekonfigurationsänderungen erforderlich, um die Cluster-, Management- und Netzwerkverbindung nach dem Neustart wiederherzustellen. Wenn die Cluster-Ports von node1 nicht direkt den Cluster-Ports von node3 zugeordnet sind, kann node3 nach einem Neustart möglicherweise nicht automatisch wieder dem Quorum beitreten, bis eine Softwarekonfigurationsänderung vorgenommen wurde, um die Cluster-LIFs auf den korrekten physischen Ports zu hosten.
...
|===
+
Weitere Informationen zu den Schritten zum Erhalt dieser Informationen finden Sie unter link:record_node1_information.html["Node1-Informationen aufzeichnen"].
...
----
+
Gehen Sie zu link:other-references.html["Quellen"], um eine Verknüpfung zu _Netzwerkmanagement_ oder der _ONTAP 9 Command reference_ für weitere Informationen herzustellen.
