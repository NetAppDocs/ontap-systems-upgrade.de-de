...
[role="lead"]
Die physischen Ports auf node2 müssen korrekt den physischen Ports auf node4 zugeordnet werden. Dadurch kann node4 nach dem Upgrade mit anderen Knoten im Cluster und mit dem Netzwerk kommunizieren.
...
.Über diese Aufgabe
Die Porteinstellungen können je nach Knotenmodell variieren. Sie müssen die Port- und LIF-Konfiguration des ursprünglichen Knotens mit der geplanten Konfiguration des neuen Knotens kompatibel machen. Dies liegt daran, dass der neue Knoten beim Start dieselbe Konfiguration erneut abruft. Das bedeutet, wenn Sie node4 starten, versucht Data ONTAP, LIFs auf denselben Ports zu hosten, die auf node2 verwendet wurden.

Wenn die physischen Ports von Knoten 2 nicht direkt den physischen Ports von Knoten 4 zugeordnet sind, sind daher Softwarekonfigurationsänderungen erforderlich, um die Cluster-, Management- und Netzwerkverbindung nach dem Neustart wiederherzustellen. Wenn die Cluster-Ports von Knoten 2 nicht direkt den Cluster-Ports von Knoten 4 zugeordnet sind, kann Knoten 4 nach einem Neustart möglicherweise nicht automatisch wieder dem Quorum beitreten, bis eine Softwarekonfigurationsänderung vorgenommen wurde, um die Cluster-LIFs auf den korrekten physischen Ports zu hosten.
...
+
Weitere Informationen finden Sie, indem Sie link:other-references.html["Quellen"] auf _Network Management_ oder die _ONTAP 9 Command reference_ verlinken und zu _Configuring failover settings on a LIF_ gehen.
