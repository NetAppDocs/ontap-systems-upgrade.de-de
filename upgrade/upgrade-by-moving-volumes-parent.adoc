...
summary: Bei einem Upgrade durch Verschieben von Volumes bereiten Sie die ursprünglichen Knoten vor und fügen die neuen Knoten dem Cluster hinzu. Sie verschieben die Volumes auf die neuen Knoten, konfigurieren die LIFs und entfernen die ursprünglichen Knoten aus dem Cluster. Ein Upgrade durch Verschieben von Volumes ist ein unterbrechungsfreies Verfahren. 
---
= Erfahren Sie mehr über Upgrades durch das Verschieben von Volumes
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Um ein Upgrade durch Verschieben von Volumes durchzuführen, bereiten Sie die ursprünglichen Knoten vor und fügen die neuen Knoten dem Cluster hinzu. Sie verschieben Volumes auf die neuen Knoten, konfigurieren LIFs und entfernen die ursprünglichen Knoten aus dem Cluster.
...
.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-7.png["Sieben"]link:upgrade-remove-original-nodes.html["Entfernen Sie die ursprünglichen Knoten aus dem Cluster."]
[role="quick-margin-para"]
Nachdem die Volumes auf die neuen Knoten verschoben wurden, entfernen Sie die ursprünglichen Knoten aus dem Cluster. Wenn Sie einen Knoten entfernen, wird die Konfiguration des Knotens gelöscht und alle Festplatten werden initialisiert.
...
Nachdem Sie sich über Upgrades durch das Verschieben von Volumes informiert haben, link:upgrade-prepare-when-moving-volumes.html["Vorbereitung auf das Upgrade"].
