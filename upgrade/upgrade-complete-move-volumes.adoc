...
summary: 'Um das Upgrade durch Verschieben von Volumes abzuschließen, ist es erforderlich, den Service Processor (SP) zu konfigurieren, neue Lizenzen zu installieren und AutoSupport einzurichten. Möglicherweise ist auch die Einrichtung von Storage oder Volume Encryption sowie die Konfiguration der FC- oder NCA-Ports erforderlich.' 
---
