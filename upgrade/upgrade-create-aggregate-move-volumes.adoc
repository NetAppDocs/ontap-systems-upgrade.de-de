...
+
Sowohl das neue Aggregat als auch das alte Aggregat, aus dem das Volumen verschoben wird, müssen sich im selben SVM befinden.
...
Nach dem Erstellen eines Aggregats und dem Verschieben von Volumen auf die neuen Knoten, link:upgrade-move-lifs-to-new-nodes.html["Nicht-SAN-Daten-LIFs und Cluster-Management-LIFs auf die neuen Knoten verschieben"].
