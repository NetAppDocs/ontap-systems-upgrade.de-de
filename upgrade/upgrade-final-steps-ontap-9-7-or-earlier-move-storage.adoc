...
summary: 'Um das Upgrade durch Verschieben des Speichers abzuschließen, müssen ungenutzte Ports und LIFs von den neuen Knoten gelöscht, das Storage-Failover oder die Hochverfügbarkeit wieder aktiviert, der Service Processor (SP) konfiguriert, neue Lizenzen installiert und AutoSupport eingerichtet werden. Möglicherweise ist auch die Einrichtung von Storage- oder Volume-Verschlüsselung sowie die Konfiguration der FC- oder CNA-Ports erforderlich.' 
---
