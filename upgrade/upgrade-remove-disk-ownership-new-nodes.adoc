...
. Geben Sie an der Bootmenü-Eingabeaufforderung  `9a` ein und drücken Sie dann die Eingabetaste.
+
...
Nach dem Entfernen der Eigentumsrechte an den an die neuen Knoten angeschlossenen Datenträgern, link:upgrade-reset-default-configuration-node3-and-node4.html["Die Standardkonfiguration auf den neuen Knoten zurücksetzen"].
