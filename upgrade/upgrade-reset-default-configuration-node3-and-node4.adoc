...
. Geben Sie an der Bootmenü-Eingabeaufforderung  `wipeconfig` ein und drücken Sie anschließend die Eingabetaste.
+
...
----
+
Das System startet das `wipeconfig` Verfahren und fährt neu hoch. Nach Abschluss des Verfahrens kehrt das System zum Startmenü zurück.

. Geben Sie im Bootmenü 8 ein, um den Knoten neu zu starten, und drücken Sie während des AUTOBOOT Strg+C, um den Knoten an der LOADER-Eingabeaufforderung anzuhalten.


.Was kommt als Nächstes?
Nach dem Zurücksetzen der Standardkonfiguration auf den neuen Knoten, link:upgrade-install-new-nodes.html["Die neuen Knoten installieren"].
