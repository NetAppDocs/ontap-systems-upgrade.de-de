...
.Schritte
. Schalten Sie die Stromversorgung von node3 ein und geben Sie dann sofort Strg+C am Konsolenterminal ein, um auf die LOADER-Eingabeaufforderung zuzugreifen.
+
...
+
Geben Sie gegebenenfalls Benutzernamen und Passwort ein oder wählen Sie Enter, um fortzufahren.
...
Nach dem Einrichten der neuen Knoten optional link:upgrade-move-internal-drives.html["Interne Laufwerke vom ursprünglichen Knoten verschieben"] oder link:upgrade-convert-node-to-shelf.html["Den ursprünglichen Knoten in ein Laufwerksgehäuse konvertieren"]. Dann link:upgrade-attach-shelves-reassign-disks.html["Speicherregale anschließen und die Festplattenzugehörigkeit neu zuweisen"].
